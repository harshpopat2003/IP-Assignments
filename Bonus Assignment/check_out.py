...
def checking_out():
    ...
    file1=open("guest_list_all.csv","r")
    file2=open("fake_guest_list_all.csv","a")
    guest=[]
    guest=csv.reader(file1)
    ...
    for row in guest:
        if row==[]:
            ...
            wr=csv.writer(file2)
            wr.writerow([row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],current_time,d1])
            mylabel4=Label(root,text="Check Out Date:",font="times 15 bold",fg='white',bg='black')
            mylabel4.place(x=650,y=440)
            mylabel5=Label(root,text=d1,font="times 15 bold",fg='white',bg='black')
            mylabel5.place(x=800,y=440)
            mylabel6=Label(root,text="Check Out Time:",font="times 15 bold",fg='white',bg='black')
            mylabel6.place(x=650,y=470)
            mylabel7=Label(root,text=current_time,font='times 15 bold',fg='white',bg='black')
            mylabel7.place(x=800,y=470)
        else:
            wtr=csv.writer(file2)
            wtr.writerow([row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9]])
    file1.close()
    file2.close()
    os.remove("guest_list_all.csv")
    os.rename("fake_guest_list_all.csv","guest_list_all.csv")
...
label1=Label(root,text="YOU CHOICE WAS :   CHECK_OUT",font="times 30 bold")
...
label3=Label(text="ENTER THE ROOM NO.:",font="times 20 bold")
...
button1=Button(root,text="OK",font="times 15 bold",command=display_record)
